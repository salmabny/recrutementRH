...
export class AuthService {
  ...
  changePassword(oldPassword: string, newPassword: string): Observable<any> {
    this._isLoading.set(true);
    return this.http.post(`${this.API_URL}/change-password`, { oldPassword, newPassword }).pipe(
      tap(() => this._isLoading.set(false)),
      catchError(err => {
        this._isLoading.set(false);
        return throwError(() => ({ message: this.parseError(err, 'Erreur lors du changement de mot de passe') }));
      })
    );
  }
